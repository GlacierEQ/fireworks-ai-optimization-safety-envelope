"""Behavioral scaffold tests for OptimizationSafetyEnvelope.
...
def test_allow_path_returns_digest() -> None:
    mech = OptimizationSafetyEnvelope()
    receipt = mech.evaluate(
        OptimizationSafetyEnvelopeRequest(subject_id="a", payload={"x": 1}, budget=1.0)
    )
...
def test_refuse_missing_subject() -> None:
    mech = OptimizationSafetyEnvelope()
    receipt = mech.evaluate(OptimizationSafetyEnvelopeRequest(subject_id="  ", payload={}, budget=1.0))
...
def test_refuse_non_positive_budget() -> None:
    mech = OptimizationSafetyEnvelope()
    receipt = mech.evaluate(OptimizationSafetyEnvelopeRequest(subject_id="a", payload={}, budget=0.0))
    assert receipt.decision is Decision.REFUSE
...
def test_different_payloads_different_digests() -> None:
    mech = OptimizationSafetyEnvelope()
    a = mech.evaluate(OptimizationSafetyEnvelopeRequest(subject_id="a", payload={"n": 1}, budget=1.0))
    b = mech.evaluate(OptimizationSafetyEnvelopeRequest(subject_id="a", payload={"n": 2}, budget=1.0))
