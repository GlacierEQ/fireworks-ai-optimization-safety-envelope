...
class AdversarialEliteTests(unittest.TestCase):
    def _load(self):
        errors = []
        for name in ('optimization_safety_envelope', "src." + 'optimization_safety_envelope'):
            try:
    ...
    def test_refuse_bad_import_path_does_not_shadow(self):
        with self.assertRaises(ModuleNotFoundError):
            importlib.import_module("src.__elite_does_not_exist_" + 'optimization_safety_envelope')
